...
Say "3. models"; try { $mo = Invoke-RestMethod "$B/api/models" -TimeoutSec 5; "models OK: $(($mo | ConvertTo-Json -Compress -Depth 3).Substring(0, [Math]::Min(300, ($mo | ConvertTo-Json -Compress -Depth 3).Length)))"; Add "models" "PASS" ""; if ($mo.backend -eq 'codex') { Add "agents run on codex (M264)" "PASS" "" } else { Add "agents run on codex (M264)" "FAIL (backend is '$($mo.backend)' - rerun the installer, or pick Codex in the map's models panel)" "" } } catch { "models: $($_.Exception.Message)"; Add "models" "FAIL" $_.Exception.Message }
Say "4. hooks, driven by hand (SessionStart, UserPromptSubmit, Stop) - gate opened for the probe session"
$env:HARNESSMAP_SESSION_GATE = 'open'; Push-Location $App; $stamp = [DateTimeOffset]::UtcNow.ToUnixTimeSeconds(); $sid = "probe-$PID-$stamp"; $word = "cobalt$stamp"  # unique per run: a repeated id gets an empty delta; a unique codeword so an older node cannot pass the filing check
...
$p1 = ('{"session_id":"' + $sid + '","turn_id":"t1","hook_event_name":"UserPromptSubmit","prompt":"we decided the header will be ' + $word + ' because it is calmer"}' | bun run hooks/on-prompt.ts 2>&1 | Out-String)
...
('{"session_id":"' + $sid + '","turn_id":"t1","hook_event_name":"Stop","last_assistant_message":"Noted: ' + $word + ' header, chosen for calm."}' | bun run hooks/on-stop.ts 2>&1) | Out-Null
...
foreach ($i in 1..24) { Start-Sleep -Seconds 5; $raw = ''; try { $raw = (Invoke-WebRequest "$B/api/state" -TimeoutSec 5 -UseBasicParsing).Content } catch {}; $m = [regex]::Match($raw, '"content":"[^"]*' + $word + '[^"]*"'); if ($m.Success) { break } }
if ($m -and $m.Success) { "node: $($m.Value)  (after $($i*5) s)"; Add "filing (a node carrying this run's codeword)" "PASS" "$($i*5) s" } else { "node: none after 120 s"; Add "filing (a node carrying this run's codeword)" "FAIL (see the backend line and the logs below)" "" }
