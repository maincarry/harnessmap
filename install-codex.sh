...
if ! curl -s -m 2 -o /dev/null http://127.0.0.1:8790/api/state; then ( cd "${APP}" && HARNESSMAP_HOME="${HOME}/.harnessmap" bun run hooks/start-server.ts >/dev/null 2>&1 ) || true; for i in $(seq 1 40); do curl -s -m 2 -o /dev/null http://127.0.0.1:8790/api/state && break; sleep 0.25; done; fi
