...
printf '%s\n' "  1. open a terminal in any project folder and run:  codex" "  2. type  /hooks  and trust the harnessmap entries (Codex skips untrusted hooks silently; the app cannot trust them, the CLI can, and both share the setting)" "  3. start a NEW thread (CLI or app) and talk as usual - every exchange files itself onto the map"
