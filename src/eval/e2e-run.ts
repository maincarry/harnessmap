  ...
  for (const a of r.then ?? []) {
    ...
    try {
      if (a.do) {
        ...
        else if (a.do === 'brainChat') { const r = await post('/api/map-status/chat', { text: a.text }); lastAsk = r.body; check(`do brainChat (${r.status})`, r.status === 200, JSON.stringify(r.body).slice(0, 120)); const said = String(r.body?.reply ?? r.body?.text ?? r.body?.answer ?? JSON.stringify(r.body ?? {})).replace(/\s+/g, ' ').trim(); console.log(`  brain asked: ${String(a.text).slice(0, 80)}\n  brain said:  ${said.slice(0, 400)}${said.length > 400 ? '…' : ''}`); s = await state(); } // the map's mind is read, not only its status code (Jacob 2026-09-19: "monitor functions and mind")
      ...
      else if (a.statusOf) { const ns = (s.nodes ?? []).filter((x: any) => x.status !== 'removed' && match(s, x, a.statusOf)); const hit = ns.find((n: any) => rx(`^(${a.is})$`).test(n.status ?? '')); check(label, !!hit, hit ? `status=${hit.status} (${(hit.title || hit.content).slice(0, 40)})` : ns.length ? ns.map((n: any) => `${n.status}: ${(n.title || n.content).slice(0, 30)}`).join('; ') : 'no node'); } // stance semantics (sqlsugar replay): ANY matching live node with the status passes, not the first match
