...
async function runOne(file: string) {
  const p = port++;
  const proc = Bun.spawn(['bun', 'run', 'src/eval/e2e-run.ts', file, ...(keep ? ['--keep'] : [])], { env: { ...process.env, E2E_PORT: String(p) }, stdout: 'pipe', stderr: 'pipe' });
  ...
  console.log(`[${file.split('/').pop()}] ${line}${fails ? '\n' + fails : ''}`);
}
for (const f of files) {
  ...
  const pr = runOne(f).finally(() => running.delete(pr)); running.add(pr);
