...
- ONLY pure dialogue mechanics produce nothing: greetings, confirmations, "ok"/"thanks", pleasantries that carry no topic at all, and meta-instructions about the harness itself. A request to switch, resume, return to, focus on or set aside a topic is mechanics too — the map's focus and light already record it; "the user wants to resume X" is never a node (if X is absent from the map, X itself is the node, stated as a topic).
