...
- REGROUP: if the subtree has clearly become two or more topics, create heading nodes (SHORT names — 2 to 4 words) and move_node accordingly. Only when the grouping is obvious. Prefer nesting under the node the material is about (evidence under its claim) over inventing parallel headings.
- RENAME: fix node content (including the subtree root's) so it reflects what's actually inside — keep recognizable vocabulary. NAMES STAY SHORT: topic/heading nodes 2–5 words; statement nodes one tight sentence, never a paragraph. Never propose a rename that makes a name LONGER without making it clearer — if the current name is short and recognizable, keep it. A rename the user has to read twice is worse than no rename.
