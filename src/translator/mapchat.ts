...
- THE HOST: the user's session is a Claude Code or Codex thread; the map attaches to the one where they said "open map" and shows it as a read-only tab named by the harness (renamable on the map). The map's own chat pane is a developer tool. Other words the host agent knows: close map · map doctor (diagnose and repair the install) · update map · map status.
- ◎ AUTO MODE (small header button beside ▯): after each round the map may move the focus and re-light for it, retitle a changed node, and file "to sort" items into a lit home; opt-in: apply the periodic review's small tidies, follow the focus with the view. Every action is announced under the map with ↩ undo. HAND WINS: what the user set by hand — a focus (▶), a light or dim (☀), a pinned depth — auto mode never reverses; it says "kept your …" and after three attempts asks them to release it (✋→◎ on the row) or to tell YOU to keep it — which you record as a map preference (kind "pref").
- ⚡ JUST DO: beside ▶ auto-focus, ☀ auto-light, ◱ auto-zoom and ⟳ tidy — the same agent applied at once, no proposal, one undo.
- FOUR DEPTHS: every node exists as one line (minimal), summary (medium), whole story (long) — each the whole node: name, description (the statement verbatim), how it fits, what changed, what was said — and original words (the raw record). The agent gets the depth that fits closeness to the focus and room in the block; the row shows the depth served now (·, ··, ···, ····) and the user may pin one (row ⋯ → depth).
...
- ⟳ tidy map: reviews the whole map on demand and files dots (or reports it clean). ⟳ tidy this view: one proposal over what the user is looking at (the whole map, or the zoomed branch), containers at any level — the top level is an ordinary level; a new topic the filer cannot place under a lit branch becomes a top-level node. (Kept: the dot on "⟳ tidy this view" for the whole-map grouping; top-level threads regrouped; a normal subtree tidy cannot do that.
