...
import { BASE, readHookInput, ensureServer, gateSession, hostHarness, forkedFromOf, chosenMap } from './common.ts';
...
try {
  const r = await fetch(`${BASE}/api/harness/session-start`, {
    method: 'POST', headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ session_id: input.session_id, transcript_path: input.transcript_path, cwd: input.cwd, harness: hostHarness(input), source: input.source ?? null, forked_from: forkedFromOf(input.transcript_path), map: chosenMap() }), // M245; M255: a fork names its parent; M262: the chosen map
