...
5. Stop and report one line each: installer · check · hooks file. Then print for the human: "ONE MANUAL STEP Codex requires: open a terminal in any project folder, run `codex`, type `/hooks`, trust the harnessmap entries. Then start a NEW thread and say: Remember: we chose blue for the header because it is calmer. Within ~30 s a node with that decision appears on http://127.0.0.1:8790. If the CLI shows no review prompt and /hooks lists nothing, run `codex --dangerously-bypass-hook-trust` once and say the line there — that proves the hooks; report it."

Guards in place against the first install's failures: ASCII-only installer with braced variables; commit-pinned URLs (the raw CDN serves stale copies for minutes); the check runs in one command (Codex's sandbox reaps background processes between commands) and the hooks start the server themselves; /api/models never answers empty; user-level hooks (Codex does not run plugin hooks) with the plugin carrying skills only; the CLI trust step printed last; host sessions told they have their tools; the test line is a decision, not a greeting (a hello files nothing); hooks call bun by its ABSOLUTE path (the app's GUI environment has no ~/.bun/bin on PATH). Unverified: Windows (the PowerShell installer has never run on a machine; Codex hooks on Windows are unverified on OpenAI's side).
