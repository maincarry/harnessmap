...
3. If `curl -s http://127.0.0.1:<port>/api/state` fails, start the server: `echo '{}' | bun run "<plugin root>/hooks/session-start.ts"` (the plugin root is two levels above this SKILL.md; on Windows: `'{}' | bun run "<plugin root>\hooks\session-start.ts"`), then wait 3 seconds.
