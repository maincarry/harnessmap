...
1. Make this request this session's. Run ONE of these (it prints a request id; you do NOT need to repeat it, and do not mention it to the user):
   ...
   Windows PowerShell: `New-Item -ItemType Directory -Force .harnessmap | Out-Null; $k = -join ((1..16) | ForEach-Object { '{0:x}' -f (Get-Random -Maximum 16) }); Set-Content -NoNewline -Path .harnessmap\open-next -Value "$k`n$($PWD.Path)"; "harnessmap request $k"`
   (The file goes in the project's own .harnessmap folder, which you can write without asking.) How it works, for you only: the harness records this command's output in this session's transcript; the map's hook at the end of your turn finds the id there and attaches THIS session and no other.
