...
3. Start it again: `bun run "$HOME/.harnessmap/app/hooks/start-server.ts"` (Windows PowerShell: `& bun run "$env:USERPROFILE\.harnessmap\app\hooks\start-server.ts"`) — always the INSTALLED app, never a plugin cache copy. It spawns the server detached and waits for health.
